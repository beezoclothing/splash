...
export default function Categories() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-sm text-gray-500 mb-4">Categories become nav menu items in the storefront.</p>

      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>{['Name','Slug','Sort','Actions'].map(h => <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">{h}</th>)}</tr>
          </thead>
        ...
        {categories.length === 0 && <p className="text-center py-8 text-gray-400 text-sm">No categories yet.</p>}
      </div>
